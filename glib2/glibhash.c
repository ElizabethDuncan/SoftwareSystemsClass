...
void data_destroyed(gpointer data) {
	free(data);
}
...
int main (int argc, char** argv)
{
    ...
    gchar **array;
    gchar line[128];
    GHashTable* hash = g_hash_table_new_full (g_str_hash, g_str_equal, (GDestroyNotify)data_destroyed,(GDestroyNotify)data_destroyed);
    int i;
    ...
    GSequence *seq = g_sequence_new ((GDestroyNotify)data_destroyed);
